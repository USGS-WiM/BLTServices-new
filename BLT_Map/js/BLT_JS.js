...
function init() {
	...
	//declaration of layers. http://54.84.207.203/arcgis/rest/services/BLT/BLT_PULAsRelated/MapServer
	PendingPULALayer = new esri.layers.ArcGISDynamicMapServiceLayer("http://blt.wim.usgs.gov/arcgis/rest/services/BLT/BLT_PULAsRelated/MapServer", { "visible": false, "opacity": 0.5 });
	CreatedPULALayer = new esri.layers.ArcGISDynamicMapServiceLayer("http://blt.wim.usgs.gov/arcgis/rest/services/BLT/BLT_PULAsRelated/MapServer", { "visible": false, "opacity": 0.5 });
	PublishedPULALayer = new esri.layers.ArcGISDynamicMapServiceLayer("http://blt.wim.usgs.gov/arcgis/rest/services/BLT/BLT_PULAsRelated/MapServer", { "visible": false, "opacity": 0.5 });
	EffectivePULALayer = new esri.layers.ArcGISDynamicMapServiceLayer("http://blt.wim.usgs.gov/arcgis/rest/services/BLT/BLT_PULAsRelated/MapServer", { "visible": false, "opacity": 0.5 });
	ExpirePULALayer = new esri.layers.ArcGISDynamicMapServiceLayer("http://blt.wim.usgs.gov/arcgis/rest/services/BLT/BLT_PULAsRelated/MapServer", { "visible": false, "opacity": 0.5 });
	...
	//HighlightLayer = new esri.layers.FeatureLayer("http://54.84.207.203/arcgis/rest/services/BLT/BLT_PULAsRelated/MapServer/1", { "visible": false });
    ...
    if (!eventIDExists) {
	    ...
	    layerDefinitions[1] = "CREATED_TIME_STAMP <= timestamp '" + formatDate + "' AND (PUBLISHED_TIME_STAMP IS NULL AND EXPIRED_TIME_STAMP IS NULL)";
	    ...
	    layerDefinitions[2] = "PUBLISHED_TIME_STAMP <= timestamp '" + formatDate + "' AND ((EFFECTIVE_DATE > timestamp '" + formatDate + "') OR (EFFECTIVE_DATE IS NULL)) AND EXPIRED_TIME_STAMP IS NULL";
	    ...
	    layerDefinitions[3] = "EFFECTIVE_DATE <= timestamp '" + formatDate + "' AND PUBLISHED_TIME_STAMP <= timestamp '" + formatDate + "' AND ((EXPIRED_TIME_STAMP >= timestamp '" + monthYear + "') OR (EXPIRED_TIME_STAMP IS NULL))";
        //expired (Expire Date is before today)
	    layerDefinitions[4] = "(EXPIRED_TIME_STAMP <= timestamp '" + monthYear + "')";
    ...
    if (!eventIDExists) {
        ...
        $.ajax({
            ...
            url: "https://blt.wim.usgs.gov/BLTProxy/proxy.ashx?https://blt.wim.usgs.gov/BLTServices/Events",
            //url: "https://blt.wim.usgs.gov/BLTServices/Events",
            type: 'GET',
            dataType: 'xml',
        ...
        $.ajax({
            url: "https://blt.wim.usgs.gov/BLTProxy/proxy.ashx?https://blt.wim.usgs.gov/BLTServices/ActiveIngredients?publishedDate=" + datetime,
            //url: "https://blt.wim.usgs.gov/BLTServices/ActiveIngredients?publishedDate=" + datetime,
            type: 'GET',
            dataType: 'xml',
            success: function (response) {
                if (!isIE) {
    ...
    identifyTask = new esri.tasks.IdentifyTask("http://blt.wim.usgs.gov/arcgis/rest/services/BLT/BLT_PULAsRelated/MapServer");
    ...
    function executePULATask(evt) {
        ...
        deferredResult.addCallback(function (response) {
            ...
            if (response.length > 1) {
                //[Pending,Created,Published,Effective,Expired] -- true/false if checkbox is checked
                var ckBoxArray = [
                    document.getElementById("dijit_form_CheckBox_4").checked,
                    document.getElementById("dijit_form_CheckBox_3").checked,
                    document.getElementById("dijit_form_CheckBox_2").checked,
                    document.getElementById("dijit_form_CheckBox_1").checked,
                    document.getElementById("dijit_form_CheckBox_0").checked
                ];

                if (eventIDExists) {
                    //this is contributor view, only show created click
                    ckBoxArray = [false, true, false, false, false];
                    for (var r = 0; r < response.length; r++) {
                        if (response[r].feature.attributes.EXPIRED_TIME_STAMP != "null")
                            response.splice(r, 1);
                        else if (response[r].feature.attributes.EFFECTIVE_DATE != "null")
                            response.splice(r, 1);
                        else if (response[r].feature.attributes.PUBLISHED_TIME_STAMP != "null")
                            response.splice(r, 1);
                        else if (response[r].feature.attributes.PULA_SHAPE_ID == "null")
                            response.splice(r, 1);
                    }
                }
                else {
                    for (var i = 0; i < ckBoxArray.length; i++) {
                        if (ckBoxArray[i] == false) {
                            //i = 0--pend, 1--create, 2--pub, 3--effect, 4--expire
                            switch (i) {
                                case 4:
                                    //remove response with feature.attributes.EXPIRED_TIME_STAMP != "null"
                                    for (var r = 0; r < response.length; r++) {
                                        if (response[r].feature.attributes.EXPIRED_TIME_STAMP != "null")
                                            response.splice(r, 1);
                                    }
                                    break;
                                case 3:
                                    //remove if response with feature.attributes.EFFECTIVE_DATE != "null"
                                    for (var re = 0; re < response.length; re++) {
                                        if (response[re].feature.attributes.EFFECTIVE_DATE != "null")
                                            response.splice(re, 1);
                                    }
                                    break;
                                case 2:
                                    //remove response with feature.attributes.PUBLISHED_TIME_STAMP != "null"
                                    for (var res = 0; res < response.length; res++) {
                                        if (response[res].feature.attributes.PUBLISHED_TIME_STAMP != "null")
                                            response.splice(res, 1);
                                    }
                                    break;
                                case 1:
                                    //remove response with feature.attributes.CREATED_TIME_STAMP != "null"
                                    for (var resp = 0; resp < response.length; resp++) {
                                        if (response[resp].feature.attributes.CREATED_TIME_STAMP != "null")
                                            response.splice(resp, 1);
                                    }
                                    break;
                                case 0:
                                    //remove response with feature.attributes.PULA_SHAPE_ID == "null"
                                    for (var respo = 0; respo < response.length; respo++) {
                                        if (response[respo].feature.attributes.PULA_SHAPE_ID == "null")
                                            response.splice(respo, 1);
                                    }
                                    break;
                            }
                        }
                ...
                if (response.length > 1) {
                    ...
                    return dojo.map(response, function (result) {
                        ...
                        var ShapeID = feature.attributes.PULASHAPEI;
    ...
    window.top.GetEventPULAs(eventID, function (result) {
        ...
        if (returningEventPULAs != undefined) {
            ...
            for (var i = 0; i < returningEventPULAs.length; i++) {
                if (returningEventPULAs[i].Expired == null) 
            ...
            while (i < ShapeIDarray.length) {
                if (i == ShapeIDarray.length - 1) {
                    layerDefinitions[1] += "PULASHAPEI = " + ShapeIDarray[i];
                }
                else {
                    layerDefinitions[1] += "PULASHAPEI = " + ShapeIDarray[i] + " or ";
                }
                i++;
            }
            PendingPULALayer.setLayerDefinitions(layerDefinitions);
            ...
            if (PublishedSHPIDs.length > 0) {
                while (i < PublishedSHPIDs.length) {
                    if (i == PublishedSHPIDs.length - 1) {
                        layerDefinitions[2] += "PULASHAPEI = " + PublishedSHPIDs[i];
                    }
                    else {
                        layerDefinitions[2] += "PULASHAPEI = " + PublishedSHPIDs[i] + " or ";
                    }
                    i++;
                }
            }
            ...
            if (EffectiveSHPIDs.length > 0) {
                ...
                while (i < EffectiveSHPIDs.length) {
                    if (i == EffectiveSHPIDs.length - 1) {
                        layerDefinitions[3] += "PULASHAPEI = " + EffectiveSHPIDs[i];
                    }
                    else {
                        layerDefinitions[3] += "PULASHAPEI = " + EffectiveSHPIDs[i] + " or ";
                    }
                    i++;
                }
            }
            ...
            if (ExpiredSHPIDs.length > 0) {
                ...
                while (i < ExpiredSHPIDs.length) {
                    if (i == ExpiredSHPIDs.length - 1) {
                        layerDefinitions[4] += "PULASHAPEI = " + ExpiredSHPIDs[i];
                    }
                    else {
                        layerDefinitions[4] += "PULASHAPEI = " + ExpiredSHPIDs[i] + " or ";
                    }
                    i++;
                }
            }
...
function ZoomToExtent(layerDefinitions) {
    //PULA_queryTask = new esri.tasks.QueryTask("http://54.84.207.203/arcgis/rest/services/BLT/BLT_PULAsRelated/MapServer/0");
    PULA_queryTask = new esri.tasks.QueryTask("http://blt.wim.usgs.gov/arcgis/rest/services/BLT/BLT_PULAsRelated/MapServer/0");
